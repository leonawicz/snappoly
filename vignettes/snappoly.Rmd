...
```{r example_run, echo=FALSE, results="hide", cache=TRUE}
e <- extent(aklcc)
...
dev.off()
png("man/figures/README-plot_rasterize-1.png", width = 600, height = 500)
par(mar = c(0, 0, 0, 0), xaxs = "i", xpd = FALSE, yaxs = "i")
plot(r, col = clrs)
dev.off()
```

```{r plot_rasterize_pkg, echo=FALSE}
library(snapgrid)
```

```{r plot_rasterize, eval=FALSE, cache=TRUE}
library(snapgrid)
e <- extent(aklcc)
r <- rasterize(aklcc, swflam) %>% mask(swflam)
plot(r, col = clrs)
```

<p style="text-align:center;"><img src="man/figures/README-plot_rasterize-1.png"></p>

For a similar introduction to `snapgrid`, see the corresponding [vignette](http://leonawicz.github.io/snapgrid/docs/articles/snappoly.html) to get started.
